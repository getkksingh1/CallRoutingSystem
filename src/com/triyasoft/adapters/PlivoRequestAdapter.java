...
public class PlivoRequestAdapter {
	...
	public static boolean isFlatBillingRate =  AppSettingsDao.isFlatBillingRate();
	...
	public CallModel createCallModelAdapterAtAnswer(HttpServletRequest request) {
		   ...
		   CallModel callModel = new CallModel();
			...
			toNumber =  toNumber.replace("+", "").replaceAll(" ", "").replaceAll("-", "");
	...
	public CallModel createCallModelAdapterAtHangup(HttpServletRequest request) throws CallModelNotInMapException {	
	...
	CallModel callModel = CallRoutingServiceV2.runningCallsMapByUUID.get(callUUID);
	
	
	if(callModel == null) {
		callModel = CallsDaoService.getCallByUUID(callUUID) ;
		if(callModel == null)
		  throw  new CallModelNotInMapException(callUUID+" :::CallUUID can not be found in runningCallsMapByUUID and database both");
	}
	...
	if(billDuration != null && totalCost !=null && billDuration.trim().length() >0) {
		double numberOfMinutes = (Double.parseDouble(billDuration))/60;
		double totalCostDouble = Double.parseDouble(totalCost) +(numberOfMinutes*outboundCostPerMinute);
	...
	callModel.setStartTime(CallRoutingServiceV2.convertPlivoDateToJavaDate(startTime));
	callModel.setEndTime(CallRoutingServiceV2.convertPlivoDateToJavaDate(endTime));
	callModel.setAnswerTime(CallRoutingServiceV2.convertPlivoDateToJavaDate(answerTime));
