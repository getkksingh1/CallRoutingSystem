	 ...
	 @Override
	    protected void service(HttpServletRequest request, HttpServletResponse response)
			...
			if("answer".equals(requestType)) {
				System.out.println("Getting Inside CallCenterInboundServlet answer ");
		        ...
		        String sipEndPointtoCall = CallcenterCallAllocationService.getAllocatedEndPoint();
		        ...
		        CallcenterCallAllocationService.markExtensionBusy(sipEndPointtoCall);
				...
				CallcenterCallAllocationService.runningInboundCalls.add(callcenterInboundCall);
				CallcenterCallAllocationService.runningInboundCallsMap.put(uuid, callcenterInboundCall);
		        ...
		        String responseToPlivo = "<Response><Dial callerId=\""+fromNumber+"\"><User>"+sipEndPointtoCall+"</User></Dial></Response>";
	        	response.addHeader("Content-Type", "text/xml");
			...
			if("hangup".equals(requestType)) {
					System.out.println("Getting Inside CallCenterInboundServlet hangup ");
    				printRequestparameters(request);
    				AuditDao.auditHttpRequest("CallCenterInboundServletHangup", request);
				  
    				String uuid = request.getParameter("CallUUID");

				    CallcenterInboundCall callcenterInboundCall =   CallcenterCallAllocationService.runningInboundCallsMap.get(uuid);
				    ...
				    String hangupCause = request.getParameter("HangupCause"); //Add to DB

				    String billDuration = request.getParameter("BillDuration");
				    String billRate = request.getParameter("BillRate");
				    String duration = request.getParameter("Duration");

				    String callStatusAtHangup = request.getParameter("CallStatus");//Add to DB
				    
				    CallcenterInboundCallsDao.updateInboundCall(totalCost,billDuration,billRate,duration,hangupCause,callStatusAtHangup,uuid);
		
				    CallcenterCallAllocationService.runningInboundCalls.remove(callcenterInboundCall);
