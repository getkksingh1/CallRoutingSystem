...
public class PlivoInBoundCallManagementServelet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		if ("answer".equals(requestType)) {
			System.out
					.println("Getting Inside PlivoInBoundCallManagementServelet answer ");
			counter++;
			...
			try {
				...
				if (fromNumber.equals("Restricted")
						|| fromNumber.equals("anonymous")
						|| (fromNumber.trim().length() == 4)) {
			...
			if (callModel.getError_code() != null) {
				...
				if ("E005".equals(callModel.getError_code())) {
					AuditDao.auditRecord(
							"PlivAnswerNonBussinessHours",
							callModel.getUuid() + " :::::"
			...
			if (buyerNumber != null && buyerNumber.startsWith("sip:")) {
				...
				String responseToPlivo = "<Response><Dial callerId=\""
						+ fromNumber + "\"><User>" + buyerNumber
						+ "</User></Dial></Response>";
			...
			response.getWriter()
					.println(
							" <?xml version=\"1.0\" encoding=\"UTF-8\"?><Response><Conference beep='false' endConferenceOnExit='true' maxMembers='2' startConferenceOnEnter='true' waitSound='"
									+ holdMusicURL
									+ "'>"
									+ conferenceID
									+ "</Conference></Response>");
			...
			params.put("hangup_url", "http://" + AppSettingsDao.getServerIP()
					+ "/" + appContext
					+ "/plivooutboundcall?requestType=hangup&legBId=" + legBId
					+ "&conferenceID=" + conferenceID + "&parentuuid=" + uuid);
			params.put("ring_url", "http://" + AppSettingsDao.getServerIP()
					+ "/" + appContext
					+ "/plivooutboundcall?requestType=ring&legBId=" + legBId
					+ "&conferenceID=" + conferenceID + "&parentuuid=" + uuid);
			params.put("answer_url", "http://" + AppSettingsDao.getServerIP()
					+ "/" + appContext
					+ "/plivooutboundcall?requestType=answer&legBId=" + legBId
					+ "&conferenceID=" + conferenceID + "&parentuuid=" + uuid);
		...
		if ("holdmusic".equals(requestType)) {
			System.out
					.println("Getting Inside PlivoInBoundCallManagementServelet holdmusic ");
			printRequestparameters(request);

			AuditDao.auditHttpRequest("plivoinboundcallholdmusic", request);
			response.addHeader("Content-Type", "text/xml");
			response.getWriter().println(
					"<?xml version=\"1.0\" encoding=\"UTF-8\"?><Response><Play loop='10000'>"
							+ ringback_audio_location + "</Play></Response>");

		}

		if ("hangup".equals(requestType)) {
			System.out
					.println("Getting Inside PlivoInBoundCallManagementServelet hangup ");
		...
		if ("disconnectCall".equals(requestType)) {
			...
			parameters.put("call_uuid", callUUID); // UUID of the call to be
													// hung up
	...
	private String recordCall(String uuid) {
		...
		parameters1.put("time_limit", "600"); // Max recording duration in
												// seconds
