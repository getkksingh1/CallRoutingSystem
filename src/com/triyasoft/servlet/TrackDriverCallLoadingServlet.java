...
public class TrackDriverCallLoadingServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		if ("callsdata".equals(requestType)) {
			...
			CallsResponse callsResponse = gson.fromJson(jsonStr,
					CallsResponse.class);
			...
			for (Call trackDriveCall : trackDrivecalls) {
				...
				Date callLandingTime = convertToDate(trackDriveCall
						.getCreated_at());
				...
				PhoneNumber phoneNumber = PhoneNumberDaoService
						.getPhoneNumberbyCallerId(callModel.getNumber_called());
				callModel.setTraffic_source_id(phoneNumber
						.getTraffic_source_id());

				// Setting up Buyer

				if (trackDriveCall.getConnected_to() != null
						&& trackDriveCall.getConnected_to().trim().length() > 0) {

					Buyer buyer = BuyerDaoService
							.loadBuyerByPhoneNumber(callModel.getConnected_to());
					...
					callModel.setTraffic_source_revenue(phoneNumber
							.getCostpercall());
