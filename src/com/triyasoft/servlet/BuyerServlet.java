...
public class BuyerServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		if ("loadbyerbyBuyerId".equals(requestType)) {
			...
			Buyer buyer = BuyerDaoService.getBuyerByByerId(Integer
					.parseInt(buyerId));
			Gson gson = new Gson();
		...
		if ("updateBuyer".equals(requestType)) {
			...
			String concurrency_cap_limit = request
					.getParameter("concurrency_cap_limit");
			System.out
					.println("concurrency_cap_limit:" + concurrency_cap_limit);
			...
			BuyerDaoService.updateBuyer(buyer_id, buyer_name, buyer_number,
					weight, tier, concurrency_cap_limit, bid_price, is_active,
					buyer_daily_cap, ring_timeout);
		...
		if ("addBuyer".equals(requestType)) {
			...
			Buyer buyer = BuyerDaoService.addBuyer(buyer_name, buyer_number,
					weight, tier, concurrency_cap_limit, bid_price, is_active,
					buyer_daily_cap);
	...
	public static void main(String[] args) {
		...
		/*
		 * Map<Integer, Buyer> buyersMap =
		 * CallRoutingServiceV2.loadBuyers(false); Iterator it =
		 * buyersMap.entrySet().iterator(); while (it.hasNext()) { Map.Entry
		 * pair = (Map.Entry)it.next(); System.out.println(pair.getKey() + " = "
		 * + pair.getValue()); Buyer buyer = (Buyer) pair.getValue() ;
		 * System.out.println(buyer); it.remove(); // avoids a
		 ...
		 * CallRoutingServiceV2.loadTrafficSources(false); Iterator it =
