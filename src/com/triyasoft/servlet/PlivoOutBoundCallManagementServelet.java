...
public class PlivoOutBoundCallManagementServelet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		if ("ring".equals(requestType)) {
			...
			System.out
					.println("Getting Inside PlivoOutBoundCallManagementServelet Ring ");
			...
			CallsDaoService.createOutBoundCallRinging(calluuid, parentuuid,
					legBId, conferenceID, from, to, requestuuid, ringtime,
					callstatus);
		...
		if ("answer".equals(requestType)) {
			System.out
					.println("Getting Inside PlivoOutBoundCallManagementServelet Answer ");
			printRequestparameters(request);
			...
			CallsDaoService.updateOutBoundCallAnswer(callUUID, aLegUUID,
					billRate, aLegRequestUUID, callstatus, answerTime);
			...
			// String actionURL =
			// "https://tracktel.io/provider/plivo/legs/44477771?conferencing=ended";
			String actionURL = "http://" + AppSettingsDao.getServerIP() + "/"
					+ appContext
					+ "/plivooutboundcall?requestType=conferencingended"
					+ ampersandEscape + "legBId=" + legBId + ampersandEscape
					+ "parentuuid=" + parentuuid;
			// String callbackUrl =
			// "https://tracktel.io/provider/plivo/legs/44477771?conferencing=callback";
			String callbackUrl = "http://" + AppSettingsDao.getServerIP() + "/"
					+ appContext
					+ "/plivooutboundcall?requestType=conferencingcallback"
					+ ampersandEscape + "legBId=" + legBId + ampersandEscape
					+ "parentuuid=" + parentuuid;

			// String waitSoundurl =
			// "https://tracktel.io/provider/plivo/legs/44477770/hold_music";
			String holdMusicURL = "http://" + AppSettingsDao.getServerIP()
					+ "/" + appContext
					+ "/plivoinboundcall?requestType=holdmusic"
					+ ampersandEscape + "legAId" + legBId + ampersandEscape
					+ "parentuuid=" + parentuuid;

			String waitSoundurl = holdMusicURL;

			// String redirectURL =
			// "https://tracktel.io/provider/plivo/legs/44477771/after";
			String redirectURL = "http://" + AppSettingsDao.getServerIP() + "/"
					+ appContext + "/plivooutboundcall?requestType=hangup"
					+ ampersandEscape + "legBId=" + legBId + ampersandEscape
					+ "parentuuid=" + parentuuid;

			response.getWriter()
					.print("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Response><Conference action='"
							+ actionURL
							+ "' beep='false' callbackUrl='"
							+ callbackUrl
							+ "' "
							+ "endConferenceOnExit='true' maxMembers='2' record='true' startConferenceOnEnter='true' waitSound='"
							+ waitSoundurl
							+ "'>"
							+ conferenceId
							+ "</Conference>"
		...
		if ("conferencingcallback".equals(requestType)) {
			System.out
					.println("Getting Inside PlivoOutBoundCallManagementServelet conferencingcallback ");
			printRequestparameters(request);

			String event = request.getParameter("Event");
			String conferenceAction = request.getParameter("ConferenceAction");

			if ("ConferenceEnter".equals(event)
					|| "enter".equals(conferenceAction)) {
				...
				if (isParentCallStillLive) {
					...
					if (runningCallsMap != null) {
						...
						if (callModel != null) {
							Buyer buyer = CallRoutingServiceV2.cachedbuyersMapByPhoneNumber
									.get(toNumber);
							if (buyer != null) {
				...
				AuditDao.auditHttpRequest(
						"plivoconferencingcallbackConferenceEnter", request);
				String callUUID = request.getParameter("CallUUID");
				String conferenceFirstMember = request
						.getParameter("ConferenceFirstMember");
				String conferenceMemberID = request
						.getParameter("ConferenceMemberID");
				...
				CallsDaoService.updateOutBoundCallConferenceEnter(callUUID,
						conferenceFirstMember, conferenceMemberID,
						conferenceName, conferenceUUID, conferencestarttime);

			}

			if ("ConferenceRecordStart".equals(event)
					|| "record".equals(conferenceAction)) {
			...
			if ("ConferenceExit".equals(event)
					|| "exit".equals(conferenceAction)) {
				AuditDao.auditHttpRequest(
						"plivoconferencingcallbackConferenceExit", request);
				String callUUID = request.getParameter("CallUUID");
				String conferenceLastMember = request
						.getParameter("ConferenceLastMember");
				Date conferenceendtime = new Date();
				CallsDaoService.updateOutBoundCallConferenceEnd(callUUID,
						conferenceLastMember, conferenceendtime);
		...
		if ("conferencingended".equals(requestType)) {
			System.out
					.println("Getting Inside PlivoOutBoundCallManagementServelet conferencingended ");
			...
			CallsDaoService.updateOutBoundCallConferenceEnd2(callUUID,
					conferenceendtime);
		...
		if ("hangup".equals(requestType)) {
			...
			System.out
					.println("Getting Inside PlivoOutBoundCallManagementServelet hangup ");
			...
			CallsDaoService.updateOutBoundCallConferenceHangUp(callUUID,
					totalCost, hangupCause, callStatus, billDuration, duration,
					hangupTime);

			// Triggger the call to next Buyer in case earlier assigned buyer
			// did not received conferencingcallback or answer
			response.getWriter().print(Blank_XML_Response);
	...
	private void allocateAnotherCallIfNeeded(HttpServletRequest request) {
		...
		List<String> buyerNumbers = CallsDaoService
				.getAllBuyersWithParentUUID(parentuuid);
		...
		boolean outboundCondition = "NO_ANSWER".equals(hangupCause)
				|| "USER_BUSY".equals(hangupCause)
				|| "NO_USER_RESPONSE".equals(hangupCause);
		boolean inboundCondition = (callModel != null);

		boolean checkIfAnotherCallRequired = outboundCondition
				&& inboundCondition;
		...
		params.put("fallback_url", "http://" + AppSettingsDao.getServerIP()
				+ "/" + appContext
				+ "/plivooutboundcall?requestType=fallback&legBId=" + legBId
				+ "&conferenceID=" + conferenceID + "&parentuuid=" + uuid);
		params.put("hangup_url", "http://" + AppSettingsDao.getServerIP() + "/"
				+ appContext + "/plivooutboundcall?requestType=hangup&legBId="
				+ legBId + "&conferenceID=" + conferenceID + "&parentuuid="
				+ uuid);
		params.put("ring_url", "http://" + AppSettingsDao.getServerIP() + "/"
				+ appContext + "/plivooutboundcall?requestType=ring&legBId="
				+ legBId + "&conferenceID=" + conferenceID + "&parentuuid="
				+ uuid);
		params.put("answer_url", "http://" + AppSettingsDao.getServerIP() + "/"
				+ appContext + "/plivooutboundcall?requestType=answer&legBId="
				+ legBId + "&conferenceID=" + conferenceID + "&parentuuid="
				+ uuid);
		params.put("url", "http://" + AppSettingsDao.getServerIP() + "/"
				+ appContext + "/plivooutboundcall?requestType=answer&legBId="
				+ legBId + "&conferenceID=" + conferenceID + "&parentuuid="
				+ uuid);
	...
	public static void main(String[] args) {
		...
		LinkedHashMap<String, String> parameters = new LinkedHashMap<String, String>();
		parameters.put("call_uuid", "594d18aa-8447-11e7-994f-619d2e59f3b2/"); // UUID
																				// of
																				// the
																				// call
																				// to
