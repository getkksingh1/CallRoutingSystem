...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (user != null && user.getUserid() != null) {
		  ...
		  //  request.login(user.getUserid(), user.getPassword()); // Password should already be the hashed variant.
		    request.getSession().setAttribute("user", user);

		    if (remember) {
		        String uuid = UUID.randomUUID().toString();
		        UsersDao.save(uuid, user);
		        addCookie(response, Constans.REMEMBER_ME_COOKIE_NAME, uuid, Constans.REMEMBER_ME_COOKIE_TIME_OUT);
		    } else {
		    	UsersDao.deleteSession(user);
		        removeCookie(response, Constans.REMEMBER_ME_COOKIE_NAME);
		    }
			response.sendRedirect(ProjectUtils.getBaseURL(request)+"/dashboard.jsp");
			...
			//RequestDispatcher rd = getServletContext().getRequestDispatcher(baseURL+"/login.jsp");
