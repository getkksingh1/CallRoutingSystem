	...
	protected void service(HttpServletRequest request,
		...
		if ("updateuser".equals(requestType)) {
			...
			String password_confirmation = request
					.getParameter("user[password_confirmation]");
			String current_password = request
					.getParameter("user[current_password]");
			UserModel userModel = (UserModel) request.getSession()
					.getAttribute("user");
			String oldPass = ((UserModel) request.getSession().getAttribute(
					"user")).getPassword();

			if (password.equals(password_confirmation)
					&& oldPass.equals(current_password)) {
				changePassword = true;
			...
			String address_country = request
					.getParameter("user[address_country]");
			String address_zip = request.getParameter("user[address_zip]");
			UsersDao.updateUser(changePassword, email, password,
					password_confirmation, current_password, time_zone,
					first_name, last_name, phone_1, skype_name, address_line1,
		...
		if ("userregistration".equals(requestType)) {
			...
			String companyName = request
					.getParameter("registration[company_name]");
			...
			String skype_name = request
					.getParameter("registration[skype_name]");
			...
			String addressLine1 = request
					.getParameter("registration[address_line1]");
			leadModel.setAddressLine1(addressLine1);
			String addressLine2 = request
					.getParameter("registration[address_line2]");
			...
			String country = request
					.getParameter("registration[address_country]");
			leadModel.setCountry(country);
			String request_demo = request
					.getParameter("registration[request_demo]");
