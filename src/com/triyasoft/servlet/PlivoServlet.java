...
public class PlivoServlet extends HttpServlet  {
	...
	//public static String nonBussinessHours = "<?xml version=\"1.0\" encoding=\"utf-8\" ?><Response><Speak voice='WOMAN'>At this time all of our offices are closed, please call back during normal operating hours 7 days a week. Thanks for calling, goodbye.</Speak><Wait length='5'/><Hangup/></Response> ";
	    ...
	    protected void doGet(HttpServletRequest request, HttpServletResponse response)
	            throws ServletException, IOException {
					...
					if("reloaddata".equals(requestType)) {
						...
						response.getWriter().println("Loaded Buyers: "+ CallRoutingService.cachedBuyersList.size());
						response.getWriter().println("Loaded PhoneNumbers: "+ CallRoutingService.cachedPhoneNumbersMap.size());
						response.getWriter().println("Loaded Sources: "+ CallRoutingService.cachedTrafficsourcesMap.size());
	 ...
	 private String recordCall(String uuid) {  
	   ...
	   parameters1.put("time_limit","7200"); // Max recording duration in seconds
