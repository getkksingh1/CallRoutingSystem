	...
	protected void service(HttpServletRequest request,
		...
		if ("answer".equals(requestType)) {
			...
			try {
				...
				if (fromNumber != null && fromNumber.startsWith("sip:")) {
					...
					callModel = twilioRequestAdapter
		...
		if ("hangup".equals(requestType)) {
			...
			try {
				...
				TwilioRequestAdapter twilioRequestAdapter = new TwilioRequestAdapter();
				CallModel callModel = twilioRequestAdapter
						.createCallModelAdapterAtHangup(request);
				CallRoutingServiceV2
						.updateCallModelWithConferenceForTwillo(callModel);
			} catch (Exception e) {
				e.printStackTrace();
				StringWriter errors = new StringWriter();
				e.printStackTrace(new PrintWriter(errors));
