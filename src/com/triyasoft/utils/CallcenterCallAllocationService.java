...
public class CallcenterCallAllocationService {
	...
	public static void loadInitdata() {
		...
		for (PlivoEndpointExtention plivoEndpointExtention : extensions) {
			sipEndPointnameExtensionMap
					.put(plivoEndpointExtention.getSip_url(),
							plivoEndpointExtention);
		}

		List<Endpoint> plivoEndPoints = PlivoEndPointUtil
		...
		if (plivoEndPoints != null && plivoEndPoints.size() > 0)
			for (Endpoint endpoint : plivoEndPoints) {
				...
				PlivoEndpointExtention plivoEndpointExtention = sipEndPointnameExtensionMap
						.get(sipURL);
				if (plivoEndpointExtention != null) {

					plivoEndpointExtention
							.setSip_registered(endpoint.sipRegistered);
				}

			}

		List<CallcenterInboundCall> runningInboundCalls = CallcenterInboundCallsDao
				.loadRunningInboundCalls();
		for (CallcenterInboundCall callcenterInboundCall : runningInboundCalls) {
			runningInboundCallsMap.put(callcenterInboundCall.getUuid(),
					callcenterInboundCall);
			markExtensionBusy(callcenterInboundCall.getTosipendpoint());
		}

		List<CallcenterOutboundCall> runningOutboundCalls = CallcenterOutboundCallsDao
				.loadRunningOutboundCalls();
		for (CallcenterOutboundCall callcenterOutboundCall : runningOutboundCalls) {
			runningOutboundCallsMap.put(callcenterOutboundCall.getUuid(),
