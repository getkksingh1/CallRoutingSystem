...
public class APISanitizer {
	...
	public static void updateNoBuyers() {
		...
		for (CallModel callModel : calls) {
			...
			PhoneNumber phoneNumber = PhoneNumberDaoService
					.getPhoneNumberbyCallerId(number_called);

			double traffic_source_revenue = phoneNumber.getCostpercall();

			boolean isBuyerDuplicate = CallsDaoService.checkBuyerDup(
					customer_number, buyer_id);

			boolean isSourceDuplicate = CallsDaoService.checkTraffic_SourceDup(
					customer_number, uuid);
			...
			CallsDaoService.updateCallByUUID(buyer_id, buyer_name,
					buyer_revenue, traffic_source_revenue, buyer_number, uuid);
	...
	public static void updateNoBuyersDueToBuyerIssues() {
		...
		Map<String, Date> inboundCallsUUIDs = CallsDaoService
				.getInboundCallsUUIDsForSanitization();
		Map<String, String> buyerNameAndNumberMap = BuyerDaoService
				.getAllBuyersMapByBuyerNumberAndName();
	...
	public static void sanitizeCallsAPIData() {
		...
		for (Map.Entry<String, Date> entry : inboundCallsUUIDs.entrySet()) {
			...
			List<OutboundCallModel> outBoundCalls = CallsDaoService
					.getOutBoundCalls(parentuuid);

			int buyerConnectedTime = 0;
			int buyerConnectingTime = 0;

			if (outBoundCalls.size() == 1) {
				Date conferenceStartTime = outBoundCalls.get(0)
						.getConferencestarttime();
				Date conferenceEndTime = outBoundCalls.get(0)
						.getConferenceendtime();
				Date buyerCallAnswerTime = outBoundCalls.get(0).getAnswertime();

				if (conferenceEndTime == null)
					conferenceEndTime = outBoundCalls.get(0)
							.getOutboundcallhanguptime();

				if (conferenceStartTime != null && conferenceEndTime != null)
					buyerConnectedTime = (int) (conferenceEndTime.getTime() - conferenceStartTime
							.getTime()) / 1000;

				if (callLandingTimeOnServer != null
						&& buyerCallAnswerTime != null)
					buyerConnectingTime = (int) (buyerCallAnswerTime.getTime() - callLandingTimeOnServer
							.getTime()) / 1000;
			...
			for (OutboundCallModel outboundCallModel : outBoundCalls) {
				...
				String buyerName = buyerNameAndNumberMap.get(outboundCallModel
						.getTo());
			...
			CallsDaoService.updateCallData(parentuuid, buyerConnectedTime,
					buyerConnectingTime, callAttempts, buyers_tried,
					buyer_hangup_reasons);

			System.out.println("uuid:" + parentuuid + " buyerConnectedTime:"
					+ buyerConnectedTime + " buyerConnectingTime:"
					+ buyerConnectingTime + " callAttempts:" + callAttempts
					+ " buyers_tried:" + buyers_tried
					+ " buyer_hangup_reasons:" + buyer_hangup_reasons);
