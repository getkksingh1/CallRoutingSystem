...
public class ProjectUtils {
	...
	public static String getBaseURL(HttpServletRequest request) {
		...
		baseURL = protocol + "://" + serverName + portStr + appContext
				+ userAppContext;
	...
	public static String findCookieValue(String cookieName,
	...
	public static Map<String, String> readProperties(String propertyFilePath) {
		...
		try {
			properties.load(ProjectUtils.class
	...
	public static void getPctunerStatus(Connection conn, String productKey,
			int statusValue) {
		String query = "update save_key_macadd set status=" + statusValue
	...
	public static String getMacAddress(Connection connection, String productKey) {
		String macAdd = null;
		String query = "select mac_add from save_key_macadd where product_key='"
				+ productKey + "'";
		;
		try {
	...
	public static Connection getMySQLConnection() {
		...
		DataSource ds = null;
		try {
			ds = (DataSource) envContext.lookup("jdbc/" + currentAppContext
					+ "DB");
		} catch (NamingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static List<String> getBrowserAndOS(HttpServletRequest request) {
		...
		if (user.contains("msie")) {
			...
			// substring.split(" ")[0].replace("MSIE", "IE") + "-" +
			// substring.split(" ")[1];
	...
	public static Date getFirstInstantOfDay(String timeZoneId, Date date) {
		Calendar resultDate = Calendar.getInstance(TimeZone
	...
	public static String getDateInOtherTimeZone() {
		...
		/*
		 * Date date = new Date(); date = getFirstInstantOfDay("US/Eastern",
		 * date); DateFormat formatter = new
		 ...
		 * 
		 * //2017-06-22 21:14:36
		 * System.out.println(getFirstInstantOfDay("US/Eastern", new
		 * Date()).getTime()); formatter = new
		 * SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
		 * formatter.setTimeZone(TimeZone.getTimeZone("Europe/London")); date =
		 * null; try { date = formatter.parse("2013-04-13 00:00:00.000"); }
		 * catch (ParseException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } System.out.println("London: " +
		 * formatter.format(getFirstInstantOfDay("Europe/London", date)));
		 * System.out.println("Helsinki: " +
		 * formatter.format(getFirstInstantOfDay("Europe/Helsinki", date)));
		 */

		/*
		 * 
		 * Date date = new Date(); DateFormat formatter= new
		 ...
		 * 
		 * String deviationFromGMT = formattedDate.substring(20); String
		 * signOfDeveation = deviationFromGMT.substring(0,1); String
		 * hourDeviation = deviationFromGMT.substring(1,3); String
		 * minutesDevation = deviationFromGMT.substring(3,5);
		 * 
		 ...
		 * //2017-06-23 15:07:41
		 * 
		 * /* String[] zoneids = TimeZone.getAvailableIDs(); for (String zoneid
		 * : zoneids) { System.out.println(zoneid);
		 * 
		 * }
		 */
