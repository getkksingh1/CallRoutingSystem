...
public class BuyerSourcePreferenceFilter {
	...
	public static boolean evaluateOp(Buyer buyer, TrafficSource trafficSource ) {
		...
		List<BuyerSourcePreferenceFilter> buyerSourcePreferenceFilters = BuyerSourcePreferenceFilterDao.getFilters();
		
		
		if((buyerSourcePreferenceFilters == null) || (buyer == null) ||(trafficSource == null) )
			return true;
		
		
		boolean wantsFromBuyer = getWantsFromBuyerIds(buyerSourcePreferenceFilters, buyer.getBuyer_id(), trafficSource.getId());
		boolean dontwantsFromBuyer = getDontWantsFromBuyerIds(buyerSourcePreferenceFilters, buyer.getBuyer_id(), trafficSource.getId());

		
		
	/*	for (BuyerSourcePreferenceFilter buyerSourcePreferenceFilter : buyerSourcePreferenceFilters) {
			
			if((buyerSourcePreferenceFilter.getBuyer_id() == buyer.getBuyer_id())  && (buyerSourcePreferenceFilter.getIs_active() ==1)) {
				{
					if("Eq".equalsIgnoreCase(buyerSourcePreferenceFilter.getOperator()))
						if(buyerSourcePreferenceFilter.getSource_id() != trafficSource.getId()) {
							return false;
						}
					if("NotEq".equalsIgnoreCase(buyerSourcePreferenceFilter.getOperator()))
						if(buyerSourcePreferenceFilter.getSource_id() == trafficSource.getId()) {
							return false;
						}
				}
		...
		*/
		...
		private static boolean getDontWantsFromBuyerIds(
			...
			for (BuyerSourcePreferenceFilter buyerSourcePreferenceFilter : buyerSourcePreferenceFilters) {
				...
				if(buyerSourcePreferenceFilter.getBuyer_id() == buyer_id && buyerSourcePreferenceFilter.getSource_id() == traffic_source_id && "NotEq".equalsIgnoreCase(buyerSourcePreferenceFilter.getOperator()))
					return false;
		...
		private static boolean getWantsFromBuyerIds(
			...
			for (BuyerSourcePreferenceFilter buyerSourcePreferenceFilter : buyerSourcePreferenceFilters) {
				if(buyerSourcePreferenceFilter.getBuyer_id() == buyer_id  &&"Eq".equalsIgnoreCase(buyerSourcePreferenceFilter.getOperator())) {
