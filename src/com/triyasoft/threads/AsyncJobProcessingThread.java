...
public class AsyncJobProcessingThread extends Thread {
	...
	public static long lasttimeDashBoardDataFectched = System.currentTimeMillis();
	...
	public static boolean executionInprogressForHeaderContainer  =  false; 
	 ...
	 public static void loadData() {
		 ...
		 long duration  =  System.currentTimeMillis() - lasttimeDashBoardDataFectched;
	...
	public void run() {
		...
		if(this.taskName.equals("dashboardheader")) {
		...
		if(this.taskName.equals("dashboardmaincontainer")) {
	...
	private String getMainSummaryContent() {
		...
		if(ogc.getMaxActiveCC() == 0)
			...
			ogc.setActiveCCPercentage((((double)ogc.getActiveCC())/((double)ogc.getMaxActiveCC()))*100);
		...
		if(ogc.getMaxPausedCC() == 0)
			...
			ogc.setPausedCCPercentage(((((double)ogc.getPausedCC())/((double)ogc.getMaxPausedCC())))*100);
		...
		if(bcs.getTotalCallsToday() ==0)
			...
			bcs.setBuyerConvetedPercentageToday(((((double)bcs.getBuyerConvertedToday())/((double)bcs.getTotalCallsToday())))*100);
		...
		if(bcs.getTotaslCallsYday() == 0)
			...
			bcs.setBuyerConvertedPercentageYday(((((double)bcs.getBuyerConvertedYday())/((double)bcs.getTotaslCallsYday())))*100);
		...
		if(bcs.getTotaslCalls1Hrs() == 0)
			...
			bcs.setBuyerConvertedPercentage1Hrs(((((double)bcs.getBuyerConverted1Hrs())/((double)bcs.getTotaslCalls1Hrs())))*100);
		...
		if(bcs.getTotaslCalls4Hrs() == 0)
			...
			bcs.setBuyerConvertedPercentage4Hrs(((((double)bcs.getBuyerConverted4Hrs())/((double)bcs.getTotaslCalls4Hrs())))*100);
	 ...
	 TrafficSourcesSummary trafficSourcesSummary =	ddm.getTrafficSourcesSummary();
		
		 for (CallModel todayCall : todaysCalls) {
			int trafficSourceId =  todayCall.getTraffic_source_id();
		 TrafficSourceSummary trafficSourceSummary =	trafficSourcesSummary.getTrafficSourceSummary(trafficSourceId);
		 
		 if(trafficSourceSummary == null) {
			 trafficSourceSummary = trafficSourcesSummary.getNewTrafficSourceSummaryInstance();
			 trafficSourceSummary.setTrafficSourceId(trafficSourceId);
			 trafficSourceSummary.setTrafficSourceName(todayCall.getTraffic_source());
		 }
		 ...
		 if(duration == null || duration.length() ==0 || "null".equals(duration))
			 duration = "0";
		 
		 trafficSourceSummary.addTotalCallTime(Double.parseDouble(duration));
		 trafficSourceSummary.addToCallRevenue(todayCall.getTraffic_source_revenue());
		
		 if(todayCall.getIs_running() ==1)
			 trafficSourceSummary.incrementLiveCall();
		 
		 trafficSourceSummary.setAvgCall(trafficSourceSummary.getTotalCallTime()/((double)trafficSourceSummary.getTodayCalls()));
		 ...
		 for (CallModel yDayCall : yesterdayCalls) {
			 ...
			 if(trafficSourceSummary == null) {
				 trafficSourceSummary = trafficSourcesSummary.getNewTrafficSourceSummaryInstance();
				 trafficSourceSummary.setTrafficSourceId(trafficSourceId);
				 trafficSourceSummary.setTrafficSourceName(yDayCall.getTraffic_source());
		 ...
		 for (CallModel todayCall : todaysCalls) {
			 ...
			 if(buyerSummary == null) {
				 ...
				 buyerSummary.setBuyerConcurrencyCap(getBuyerConcurrencyCap(buyerId));
			 ...
			 buyerSummary.setAvgCalltime(buyerSummary.getTodaysCalltimeTobuyer()/((double)buyerSummary.getTodayCallstoBuyer()));
		 ...
		 List<BuyerSummary>  currentBuyersInBuyerSummary = 	 ddm.getBuyersSummary().getBuyersSummary();
		 ...
		 for (Buyer buyer : allBuyers) {
			 ...
			 if(buyer.getIs_active() == 1)
			 {
				 ...
				 if(!buyerPresent) {
					 BuyerSummary buyerSummary =  ddm.getBuyersSummary().getBuyerSummaryInstance();
					 ...
					 buyerSummary.setBuyerConcurrencyCap(buyer.getConcurrency_cap_limit());
		 ...
		 sourcesSummary.setSourceCount( sourcesSummary.getSourceSummaries().size());
		 sourcesSummary.setTotalCallCountToday(ddm.getBuyerConversion().getTotalCallsToday());
		 sourcesSummary.setTotalCallCountYday(ddm.getBuyerConversion().getTotaslCallsYday());
		 sourcesSummary.setTotalPayout(ddm.getBuyerRevenue().getBuyerRevenueToday()-ddm.getPhoneCost().getPhoneCostToday()-ddm.getProfit().getProfitToday());
		 sourcesSummary.setTotalLiveCalls(ddm.getOnGoingCalls().getOngoingCalls());
		 
		 List<TrafficSourceSummary>  summaries =   sourcesSummary.getSourceSummaries();
		 ...
		 for (TrafficSourceSummary trafficSourceSummary : summaries) {
			 ...
			 totalCalltime = totalCalltime+ trafficSourceSummary.getTotalCallTime();
		 ...
		 // //Setting dashboard buyers summary footer
		   ...
		   buyersSummary.setCurrentLiveCallsToBuyers(ddm.getOnGoingCalls().getOngoingCalls()+ddm.getBuyersSummary().getNoBuyer().getLiveCallsToBuyer());
		   buyersSummary.setTotalCallsToBuyersToday(ddm.getBuyerConversion().getBuyerConvertedToday()+ddm.getBuyersSummary().getNoBuyer().getTodayCallstoBuyer());
		   buyersSummary.setTotalCallsToBuyersYday(ddm.getBuyerConversion().getBuyerConvertedYday()+ddm.getBuyersSummary().getNoBuyer().getYdayCallstoBuyer());
		   buyersSummary.setTotalBuyerRevenue(ddm.getBuyerRevenue().getBuyerRevenueToday());
		   ...
		   List<BuyerSummary>  buyerSummaries =   buyersSummary.getBuyersSummary();
			 ...
			 for (BuyerSummary buyerSummary : buyerSummaries) {
				 ...
				 totalCalltime = totalCalltime+ buyerSummary.getTodaysCalltimeTobuyer();
				 numberofcall = numberofcall + buyerSummary.getTodayCallstoBuyer();
			 ...
			 buyersSummary.setAvgHandlingTime(ProjectUtils.convertDurationToString(callAvgTime));
		...
		Collections.sort(buyersSummary.getBuyersSummary());
		 ...
		 String daashboardPartialUI =  dashboardUIGenerator.generateDashBoardPartialUI(ddm);
	...
	private String getHeaderSummaryContent() {HeaderSummaryModel hsm = new HeaderSummaryModel();
	...
	private List<TrafficSource> getTrafficSourcesDeliveredCallsToDayORYesterday() {
		 ...
		 for (CallModel todayCall : todaysCalls) {
			 map.put( todayCall.getCall_from_source().getId(), todayCall.getCall_from_source());
			 
		 }
		 
		 for (CallModel ydayCall : yesterdayCalls) {
			 map.put( ydayCall.getCall_from_source().getId(), ydayCall.getCall_from_source());
	...
	private double getProfitOfXHrs(int hours) {
		 ...
		 for (CallModel todayCall : todaysCalls) {
			 long timeDiff = System.currentTimeMillis() - todayCall.getCallLandingTimeOnServer().getTime();
			 ...
			 double currentCallProfit = todayCall.getBuyer_revenue()-todayCall.getTraffic_source_revenue() - Double.parseDouble(phoneCost);
	...
	private double getYdayProfit() {
		 ...
		 for (CallModel yDayCall : yesterdayCalls) {
			 ...
			 double currentCallProfit = yDayCall.getBuyer_revenue()-yDayCall.getTraffic_source_revenue() - Double.parseDouble(phoneCost);
